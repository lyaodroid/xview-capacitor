...
export class CapacitorVideoPlayerWeb
    ...
    async initPlayer(
        ...
        if (mode === "fullscreen" || mode === "embedded") {
            ...
            return Promise.resolve({
                ...
                method: "initPlayer",
                message:
                    "Must provide a Mode either fullscreen or embedded)",
            });
        }
    }
    /**
    ...
    async getDuration(
        ...
        if (this._players[playerId]) {
            let duration: number = this._players[playerId].videoEl
                .duration;
            return Promise.resolve({
                method: "getDuration",
                result: true,
    ...
    async setCurrentTime(
        ...
        if (this._players[playerId]) {
            const duration: number = this._players[playerId].videoEl
                .duration;
            seekTime =
                seekTime <= duration && seekTime >= 0
                    ? seekTime
                    : duration / 2;
    ...
    async getCurrentTime(
        ...
        if (this._players[playerId]) {
            const seekTime: number = this._players[playerId].videoEl
                .currentTime;
            return Promise.resolve({
    ...
    private async _getContainerElement(
        ...
        const videoContainer: HTMLDivElement = document.createElement(
            "div"
        );
